...
*   **搜尋加速**：
    ...
    *   **搜尋轉換優化**：在子集搜尋時，先切片再進行字串轉換與小寫化，減少運算量。
    *   **結果列表排序優化**：排序時使用快取的整數 ID 欄位，避免重複的字串轉整數運算。
    *   **結果列表顯示優化**：在顯示資料時優先檢查 None，減少 pandas 函式呼叫。
