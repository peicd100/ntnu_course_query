...
#
# 核心目標（已調整）：
# - 所有模式：不實際刪除任何檔案或資料夾；只要依照本文件刪除規則「產出可刪清單（含理由）」
...
#
# 重要變更（已採用）：
# - 不使用 backup/ 備份；版本控制（git/GitHub）由使用者自行處理
# - dist/、build/ 不上傳 GitHub：不建立/不修改 .gitignore，改用 git add 的 exclude pathspec（僅寫在 README 指令段落）
# - .gitignore 若存在：禁止建立/修改/刪除（僅保留）
...
1.9 .gitignore 禁令（硬性；所有模式）
- 你不得建立、修改、刪除 .gitignore。
- 若 workspace root 中已存在 .gitignore：必須保留原樣，不得變更內容。
...
8) 你預計使用的 conda 進入方式（只列出，不得執行）：
   - *_CHATGPT：使用 5.3.1 的 cmd + activate.bat + conda activate
   - *_COPILOT：允許任何方式（含 pylanceRunCodeSnippet/conda run/conda activate/conda.exe 絕對路徑等），但必須遵守 5.2/5.4 的環境安全驗證
...
- conda 進入方式：5.3.1（cmd + activate.bat）
- 允許終端機：是

B) APP_MAIN_FREE_COPILOT
- 限制最多、流程最完整：必須執行本文件第 4～10 章全部要求（含終端機行為；但不含 git 指令；且不實際刪除）
- conda 進入方式：允許任何方式，但必須遵守 5.2/5.4 的環境安全驗證
- 允許終端機：是
...
- 以 APP_MAIN_FREE 為基礎，但：
  ...
  - NO_TERMINAL 模式下「建立 ENV_NAME」僅指「命名並記錄」；實際 conda create 必須在可終端機模式執行

D) FREE_CHATGPT / FREE_COPILOT
- 只保留最核心的「系統安全 + conda 環境安全」規定（第 1 章與第 5 章）
- 但仍必須建立/維護 README.md 與 Log.md（第 4 章）
- FREE 模式只強制：README/Log 的存在、ENV_NAME 記錄與核心禁令；以及可刪清單的輸出（若使用者要求）。
...
4.2 自動建立（硬性；所有模式都必須）
- 若 workspace root 下不存在 README.md：你必須自動建立 README.md 並寫入 4.5 骨架。

4.3 README.md 的 H1 規則（硬性）
- README.md 第一行必須為：# <workspace_root_basename>

4.4 app_main.py 檔首三引號最小化（硬性）
- app_main.py 檔首三引號只保留：
  - 完整使用說明請見 README.md（必填）
  - 所有輸入/設定預設放在 user_data/（必填）

4.5 README.md 必填章節（硬性）
README.md 必須至少包含以下章節：
...
(8) 從零開始安裝流程（必須提供兩種：手動/自動；皆可一鍵複製）
    ...
    (8.1) 手動安裝（手動 conda create + 手動 conda/pip 安裝）
    - 必須包含：
      - conda create / conda activate
      - conda install（優先）
      - 若 conda 無法取得才 pip install（需在 README 說明原因）
...
(10) 測試方式（只測 python app_main.py；兩個時點）
(11) 打包成 .exe（必填；提供可複製指令）

(12) GitHub操作指令（必填；置於 README.md 最後面；內容必須完全一致；只允許 URL 佔位符替換）
    # 初始化
    ```
    git init
    git branch -M main
...
- ENV_NAME 必須記錄在 README.md (6)。

5.3 conda 進入方式差異（CHATGPT vs COPILOT）
5.3.1 *_CHATGPT
- 使用 cmd + activate.bat：
...
5.3.2 *_COPILOT
- 方式不限制，但每次安裝/執行/鎖定檔/打包前必須能證明 python 位於 ENV_NAME。

5.4 環境驗證（僅允許終端機的模式適用；NO_TERMINAL 略過）
- python -c "import sys; print(sys.executable); print(sys.prefix)"

================================================================================
6. APP_MAIN_FREE_*：完整流程（不含 git 指令；且不實際刪除）
================================================================================
...
6.3 測試（需要終端機；必做；只測 python app_main.py；兩時點）
- (1) 改碼後、產出清單前
- (2) 打包前：先產出清單，再測

6.4 可刪清單（必做；不刪）
- 白名單外 + 判定不必要者 → 列入可刪清單（含理由/風險）
