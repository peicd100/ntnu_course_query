...
#
# 核心目標（已調整）：
# - 所有模式：不實際刪除任何檔案或資料夾；只要依照本文件規則「產出可刪清單（含理由）」並提供「由使用者執行」的刪除指令
# - APP_MAIN_FREE_*：限制最多、流程最完整（README/Log/鎖定檔/測試/可刪清單/刪除指令/打包 全部要做）
# - APP_MAIN_NO_TERMINAL_GEMINI：以 APP_MAIN_FREE 為基礎，但「終端機功能關閉」且「所有需要終端機的規定一律略過」
# - FREE_*：只保留最核心的「系統安全 + conda 環境安全」規定（但仍必須建立/維護 README.md 與 Log.md；若產出可刪清單則必須給刪除指令）
...
# - 刪除行為：Agent 不執行刪除；但必須提供「使用者可執行」的逐項刪除指令
...
- 任何原本規定「必須刪除」之處，均改為：
  - 必須列入可刪清單（含理由/風險）
  - 並輸出「由使用者執行」的逐項刪除指令（見 6.4.1）
- 你仍必須依照白名單/判定規則完成分析與清單輸出。
- 你不得在終端機執行任何刪除命令；只能輸出給使用者。
...
- 每筆至少包含：變更摘要 / 影響範圍 / 檔案變更清單 / 測試結果摘要 / 鎖定檔更新狀態 / 可刪清單摘要 / 刪除指令摘要
...
6.4.1 使用者刪除指令（硬性；Agent 只輸出不執行）
- 在完成可刪清單後，你必須額外輸出一段「供使用者執行的刪除指令」。
- 你不得執行這些指令；只能輸出給使用者。
- 指令必須滿足：
  1) 只包含可刪清單內的路徑（不得包含任何白名單項目）
  2) 必須以 workspace root 為基準（使用相對路徑或先 cd /d "<workspace_root>"）
  3) 必須提供 Windows cmd 版本（必要時可另外附 PowerShell 版本）
  4) 必須採「逐項刪除」而非模糊匹配（不得用 rd /s /q .、不得用廣泛 wildcard 造成誤刪）
  5) 若路徑包含空白或中文，必須正確加上引號
- 若任何檔案/資料夾的刪除存在高風險（風險等級=高）：
  - 指令區塊中必須把它們分組置於最後
  - 並在指令區塊外以文字提醒「高風險項目建議逐一確認」
...
- 所有「不需要終端機」的規定仍必須執行：
  ...
  - 使用者刪除指令（6.4.1；以靜態分析清單產出指令；不得執行）
